...
$query = mysqli_query($conn,"SELECT * FROM message WHERE status='unread' AND t='$id' ORDER BY id DESC")or die(mysqli_error($conn));
$getinbox = mysqli_fetch_array($query);
if($getinbox > 0){
	$msg.= "<table class='table table-bordered' width='500px'><caption>Click Subject to Read Message from Admins</caption>
	...
	do{
		$msg.= "<tr><td>".$num."</td>";
		$farmer = mysqli_query($conn,"SELECT * FROM admin WHERE adminid='$getinbox[frm]'");
		...
		if($getinbox['status'] == "unread"){
			$msg.= "<td><b style='color:#0000ff;'><a href='read2f.php?msgid=$getinbox[id]'>".$getinbox['subject']."</a></b></td>";
		}else{
 ...
 <nav>
   ...
   <nav>
    <ul  role="tablist" id="accordion" class="panel-group">
      ...
      <li><a class='active' href = "inboxf.php"><img src = "images/inbox.gif" border = "0" /></a></li>
      <li><a href = "com_mesf.php"><img src = "images/compose.gif" border = "0" /></a></li>
		...
		<br/><br/>
         <div style = "background-color:white;width:600px; border: solid 1px #333333; " >
            <div style = "background-color:#333333; color:#FFFFFF;width:600px; border: solid 1px #333333; padding:3px;text-align:center;"><h2 class="title"><a href="inboxf.php">Inbox</a></h2></div>

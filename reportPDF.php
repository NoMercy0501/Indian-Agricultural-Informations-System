...
$pdf->AddPage();
       ...
       $pdf->SetFont('Arial', 'B', 13);
		...
		$header = array("#", "FNAME", "LNAME" ,"NATIONAL ID","TELEPHONE");
      ...
      //$getdata = $db->Execute("select * from farmer where stud_id ={$_GET['recordID']}");
		 ...
		 $getdata = $db->Execute("select * from farmer ");
		  ...
		  
		 {
				...
				$mobile		= $getdata->fields["mobile"];
								
				
				$data[] = array($k,$firstname,$lastname,$idno,$mobile);
				$getdata->MoveNext();
				$k++;
		 }
		 foreach($data as $row)

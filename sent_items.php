...
$query = mysqli_query($conn,"SELECT * FROM message  WHERE status='unread' ORDER BY id DESC")or die(mysqli_error($conn));
$getinbox = mysqli_fetch_array($query);
if($getinbox > 0){
	$msg.= "<table class='table table-bordered' width='500px'><caption>Click Subject to Read Sent Messages</caption>
	...
	do{
		$msg.= "<tr><td>".$num."</td>";
		$farmer = mysqli_query($conn,"SELECT * FROM farmer WHERE farmerid='$getinbox[t]'");
		$getfarmer = mysqli_fetch_array($farmer);
		$msg.= "<td align='center'>".$getfarmer['firstname']."&nbsp;".$getfarmer['lastname']."</td>";
		$msg.= "<td><i style='color:#0000ff;'><a href='read.php?msgid=$getinbox[id]'>".$getinbox['subject']."</a></i></td>"; 
